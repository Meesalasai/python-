...
#nested loop is another way:
#mycards = []
#for r in RANKS: for every item in RANKS take all items from SUITE
#    for s in SUITE: # for all items in SUITE take every item from RANK
#        mycards.append((s,r))
...
class Player:
    ...
    def __init__(self, name, hand): #hand object instance
        self.name = name
        self.hand = hand #one os those hand objects
...
# Check for War! If there's a war?
    if c_card[1] == p_card[1]: # the ranking I want actually to compare, that's why [1], because playing cards are tuples
        war_count += 1
